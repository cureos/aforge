...
namespace Accord.Tests.Statistics
{
    ...
    public class PartialLeastSquaresAnalysisTest
    {
        ...
        public void SimplsRegressionTest()
        {
			...
			double[] expectedI = new double[]{ 60.717, -8.509, -4.362 };
        ...
        public void SimplsComputeTest()
        {
			...
			double[] expectedX = new double[]{ 0.86, 0.12, 0.00, 0.86 };
            double[] actualX = pls.Predictors.FactorProportions;
            Assert.IsTrue(expectedX.IsEqual(actualX, 0.01));

			double[] expectedY = new double[]{ 0.67, 0.13, 0.17, 0.00 };
            ...
            double[,] X0 = (double[,])pls.Source.Clone(); Tools.Center(X0, true);
            double[,] Y0 = (double[,])pls.Output.Clone(); Tools.Center(Y0, true);
		...
		*/
        ...
        public void NipalsComputeTest2()
        {
			...
			double[,] inputs = Matrix.Stack<double>(
				x1.Submatrix(idxTrain).ToArray<double>(),
				x2.Submatrix(idxTrain).ToArray<double>(),
				x3.Submatrix(idxTrain).ToArray<double>()
			).ToMatrix<double>();
			...
			double[,] test = Matrix.Stack<double>(
				x1.Submatrix(idxTest).ToArray<double>(),
				x2.Submatrix(idxTest).ToArray<double>(),
				x3.Submatrix(idxTest).ToArray<double>()
			).ToMatrix<double>();
