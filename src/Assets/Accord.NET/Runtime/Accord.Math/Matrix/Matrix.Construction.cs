...
namespace Accord.Math
{
    ...
    public static partial class Matrix
    {
        ...
        {
						/*
			T[][] c = Concatenate<T> (a.ToArray<T> (), b.ToArray<T> ());
			T[,] r = c.ToMatrix<T>();
			return r;
			*/
			...
			int rows = Math.Max (aRows, bRows);
			int cols = aCols + bCols;
		...
		/// </summary>
		/// 
		public static T[,] Stack<T>(T[,] a, T[,] b)
		{
			int aRows = a.GetLength (0);
			int aCols = a.GetLength (1);
			int bRows = b.GetLength (0);
			int bCols = b.GetLength (1);
			int rows = aRows + bRows;
			int cols = Math.Max(aCols, bCols);
        ...
        public static T[,] Expand<T>(T[,] matrix, int[] count)
        {
			...
			return expansion.ToArray().ToMatrix<T>();
        }
        #endregion
