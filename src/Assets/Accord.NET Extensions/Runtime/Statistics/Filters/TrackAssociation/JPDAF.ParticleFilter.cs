...
namespace Accord.Extensions.Statistics.Filters
{
    ...
    public static partial class JPDAF
    {
        ...
        public static double[,] Update<TFilter, TParticle, TMeasurement>(
			this IList<TFilter> particleFilters,
			...
			double falseAlarmProbability = 0.01,
            ...
            where TFilter : List<TParticle>
        {
            ...
            //measure
            double[,] probsZX;
            var probsZXn = calculateZXMatrix(
				particleFilters.Cast<IList<TParticle>>().ToList(), 
				measurements,
				likelihoodFunc,
				out probsZX, 
				(mIdx, tIdx) => 
            ...
            if(measurements.Count > 0)
            {
                ...
                {
					IEnumerable<double> weights = particleFilters[i].Select<TParticle, double>(x => x.Weight);
                    IEnumerable<TParticle> resampledParticles = resample(particleFilters[i], weights);
